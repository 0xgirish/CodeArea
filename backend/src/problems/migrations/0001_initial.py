# -*- coding: utf-8 -*-
# Generated by Django 1.11 on 2018-03-07 09:21
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion
import problems.models
import problems.storage


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Problem',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(max_length=255)),
                ('problem_code', models.CharField(max_length=100, unique=True)),
                ('slug', models.SlugField(unique=True)),
                ('statement', models.TextField()),
                ('timestamp', models.DateTimeField(auto_now_add=True)),
            ],
            options={
                'ordering': ['-timestamp'],
            },
        ),
        migrations.CreateModel(
            name='TestCase',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('input', models.FileField(storage=problems.storage.OverwriteStorage(), upload_to=problems.models.upload_input)),
                ('output', models.FileField(storage=problems.storage.OverwriteStorage(), upload_to=problems.models.upload_output)),
                ('sample', models.BooleanField()),
                ('weight', models.IntegerField(default=0)),
                ('testcase', models.IntegerField(unique=True)),
                ('problem_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='problems.Problem')),
            ],
        ),
    ]
